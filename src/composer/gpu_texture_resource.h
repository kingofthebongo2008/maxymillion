...
namespace composer
{
    namespace gpu
    {
        ...
        concurrency::task<texture_resource> create_texture_resource(ID3D11Device* d, const wchar_t* file_name)
        {
            std::wstring url(file_name);

            return concurrency::create_task([d, url]
            {
                auto texture0 = imaging::read_texture(url.c_str());
                auto gpu_texture = upload_to_gpu(d, texture0);
